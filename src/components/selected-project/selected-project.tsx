...
export function SelectedProjectsSection() {
  ...
  return (
    <section ref={sectionRef} id="selected-projects" className="bg-[#1e1e1f] relative overflow-hidden py-12 lg:py-20">
      ...
      <div
        className={`absolute bottom-20 left-20 w-8 h-8 bg-gradient-to-r from-[#FF6B35] to-[#E55A2B] rounded-full transition-all duration-2000 delay-500 ${
          isVisible ? "opacity-100 scale-100" : "opacity-0 scale-0"
        }`}
      />
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid lg:grid-cols-3 gap-6 lg:gap-8 mb-12 lg:grid-rows-1">
          {selectedProjects.map((project, index) => (
            <div
              key={project.id}
              className={`group bg-[#2a2a2b]/30 rounded-2xl overflow-hidden border border-[#FF6B35]/10 transition-all duration-700 hover:border-[#FF6B35]/30  ${
                isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
              }`}
            ...
            >
              <div className="relative">
                ...
                <div className="absolute top-4 left-4">
                  <span className="px-3 py-1 bg-[#FF6B35]/90 text-white rounded-full text-xs font-medium backdrop-blur-sm">
                    {project.category}
                  </span>
                </div>
