...
export function PreloaderProvider({ children }: { children: ReactNode }) {
  ...
  const preloadImage = useCallback(
    async (url: string, priority: "high" | "medium" | "low" = "medium"): Promise<void> => {
      ...
      const existing = preloadedResources.get(url)
  ...
  const preloadImages = useCallback(
    async (urls: string[], priority: "high" | "medium" | "low" = "medium"): Promise<void> => {
      ...
      const sortedUrls = urls.sort((a, b) => {
        const aPriority = preloadedResources.get(a)?.priority || priority
        const bPriority = preloadedResources.get(b)?.priority || priority
        return priorityOrder[aPriority] - priorityOrder[bPriority]
      })

      // Load high priority images first, then batch load others
      const highPriorityUrls = sortedUrls.filter(
        (url) => (preloadedResources.get(url)?.priority || priority) === "high",
      )
      const otherUrls = sortedUrls.filter((url) => (preloadedResources.get(url)?.priority || priority) !== "high")
  ...
  const isResourceLoaded = useCallback(
    (url: string): boolean => {
      return preloadedResources.get(url)?.loaded || false
    },
    [preloadedResources],
  )
  ...
  const getLoadingProgress = useCallback((): number => {
    const resources = Array.from(preloadedResources.values())
